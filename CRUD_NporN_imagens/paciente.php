...
if(isset($_POST['criar'])) {
    $stmt = $pdo->prepare("INSERT INTO paciente (perfil, nome, cpf, telefone) VALUES (?, ?, ?, ?)");
    $stmt->execute([$_POST['perfil'], $_POST['nome'], $_POST['cpf'], $_POST['telefone']]);
...
if(isset($_POST['atualizar'])) {
    $stmt = $pdo->prepare("UPDATE paciente SET perfil=?, nome=?, cpf=?, telefone=? WHERE id=?");
    $stmt->execute([$_POST['perfil'], $_POST['nome'], $_POST['cpf'], $_POST['telefone'], $_POST['id']]);
    header('Location: paciente.php');
    exit;
}
...
<body>
    ...
    <form method="POST">
        ...
        <p>
            <label>Foto de Perfil: <input type="text" placeholder="Cole aqui a URL da imagem" name="perfil" value="<?= $editando['perfil'] ?? '' ?>" required></label>
        </p>
        <p>
    ...
    <table border="1" cellpadding="10">
        ...
        <tr>
            <td><?= $p['id'] ?></td>
            <td>
                <img src="<?= $p['perfil'] ?>" alt="Foto de Perfil" style="width: 100px; height: 100px;">
            </td>
            <td><?= $p['nome'] ?></td>
            <td><?= $p['cpf'] ?></td>
            <td><?= $p['telefone'] ?></td>
